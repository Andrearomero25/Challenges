...
# REFATORIZACIÓN DE CÓDIGO

import time
from calendar import isleap
from funciones import calcular_dias_anios
# from funciones import *

# Eliminar la sentencia if/else de la función
def anio_bisiesto(anio):
    return isleap(anio)

# Utilizar la cláusula in en lugar de varias cláusulas or en la función
def calcular_dia_meses(mes, anio_bisiesto):
...
# Validar que la edad ingresada por el usuario es numérica:
while True:
    edad = input('Ingrese su edad: ')
    if edad.isdigit():
        edad = int(edad)
...
# Encapsular el cálculo del equivalente de la edad en días en una función separada:
...
def calcular_edad_en_dias(hora_local, anio_comienzo, anio_fin):
    ...
    for m in range(1, hora_local.tm_mon):
        dias += calcular_dia_meses(m, anio_bisiesto(hora_local.tm_year))
